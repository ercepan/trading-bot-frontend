...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErr(msg);
      // Email onaylanmamış hatası özel UX
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            {err && (
              <div className="rounded-md border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-400 space-y-2">
                <div>{err}</div>
                {showVerifyHelp && (
                  <Link
                  ...
                  >
                    <Mail className="size-3" /> Onay maili tekrar gönder →
                  </Link>
                )}
              </div>
            )}
